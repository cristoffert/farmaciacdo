...
@section('title','AbonosCompra')

@section('titulo-contenido','AbonosCompra')
        ...
        <div class="card">
            <div class="card-header">
              <!--   {{-- <h4 class="card-title"> Simple Table</h4> --}} -->
                <a href="{{ url('/abonocompra/create') }}" class="btn btn-warning btn-round">Nuevo Abono</a>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table" cellspacing="0" id="tableAbonoCompra">
                        ...
                        <tbody>
                            @foreach( $abonoCompras as $abonoCompra )
                                <tr>
                                    <td class="text-center">{{$abonoCompra -> id}}</td>
                                    <td class="text-center">{{$abonoCompra -> valor}}</td>
                                    <td class="text-center">{{$abonoCompra -> fecha}}</td>
                                    <td class="text-center">{{$abonoCompra -> fk_compra}}</td>
                                    ...
                                    <td class="td-actions text-center">
                                        <form method="post" class="delete">
                                            ...
                                            <a class='btn btn-danger btn-simple btn-xs' rel="tooltip" title="Eliminar Abono Compra #{{ $abonoCompra -> id }}" onclick="Delete('{{ $abonoCompra -> id }}','{{ $abonoCompra -> id }}')">
